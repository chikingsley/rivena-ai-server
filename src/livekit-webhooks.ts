...
function handleParticipantJoined(event: WebhookEvent) {
  console.log(`[LiveKitWebhooks] Participant joined: ${event.participant.identity} in room ${event.room.name}`);
  // Add your custom logic here
}

function handleParticipantLeft(event: WebhookEvent) {
  console.log(`[LiveKitWebhooks] Participant left: ${event.participant.identity} from room ${event.room.name}`);
  // Add your custom logic here
}

function handleTrackPublished(event: WebhookEvent) {
  console.log(`[LiveKitWebhooks] Track published: ${event.track.sid} by ${event.participant.identity} in room ${event.room.name}`);
  // Add your custom logic here
}

function handleTrackUnpublished(event: WebhookEvent) {
  console.log(`[LiveKitWebhooks] Track unpublished: ${event.track.sid} by ${event.participant.identity} in room ${event.room.name}`);
  // Add your custom logic here
}