...
export const createToken = async (
  ...
  console.log(
    `[LiveKit] Creating token for participant: ${participantName} in room: ${roomName} (ttl: ${ttl})`,
  );
  ...
  try {
    ...
    console.log(
      `[LiveKit] Token successfully created for ${participantName} (token length: ${token.length} chars)`,
    );
    return token;
  } catch (error) {
