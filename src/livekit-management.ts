...
export class LiveKitManagement {
  ...
  async listParticipants(roomName: string) {
    ...
    try {
      ...
      console.log(`[LiveKitManagement] Found ${participants.length} participants in room: ${roomName}`);
      return participants;
    } catch (error) {
  ...
  async updateParticipantPermissions(roomName: string, identity: string, permissions: {
    canPublish?: boolean;
  ...
  async updateParticipantMetadata(roomName: string, identity: string, metadata: Record<string, any>) {
  ...
  async muteParticipantTrack(roomName: string, identity: string, trackSid: string, muted: boolean) {
    console.log(`[LiveKitManagement] ${muted ? 'Muting' : 'Unmuting'} track ${trackSid} for ${identity} in room: ${roomName}`);
    try {
      await this.roomService.mutePublishedTrack(roomName, identity, trackSid, muted);
      console.log(`[LiveKitManagement] ${muted ? 'Muted' : 'Unmuted'} track ${trackSid} for ${identity} in room: ${roomName}`);
      return true;
    } catch (error) {
